...
/// 取得我參與的專案查詢處理器
/// </summary>
public class GetParticipatedProjectsQueryHandler : IRequestHandler<GetParticipatedProjectsQuery, PagedResult<ProjectListDto>>
{
    private readonly IApplicationDbContext _context;

    public GetParticipatedProjectsQueryHandler(IApplicationDbContext context)
    {
        _context = context;
    }

    public async Task<PagedResult<ProjectListDto>> Handle(GetParticipatedProjectsQuery request, CancellationToken cancellationToken)
    {
        var query = _context.Projects
            .Include(p => p.Members.Where(m => m.RemovedAt == null))
            .Include(p => p.Forms)
            .AsNoTracking()
            .Where(p => p.Members.Any(m => m.UserId == request.CurrentUserId && m.RemovedAt == null));
